...
if (isset($_POST['sumitBtn'])) {
  ...
  $emailquery = "SELECT * FROM message_us WHERE email = '$email' ";
  $sqliquery = mysqli_query($conn, $emailquery);
    if (mysqli_num_rows($sqliquery) > 0) {
     ...
     $insertquery = "INSERT INTO message_us(name, email, mobile_no, city, message)VALUES('$name','$email','$mobileno','$city','$message')";
...
function emailsmtp(){
        ...
        $message .= "<tr><td><strong>Mobile No:</strong> </td><td>" . strip_tags($_POST['mobileno']) . "</td></tr>";
